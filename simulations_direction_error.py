...
def simulations_direction_error(channel_type, params, snr, num_sim=100):
    ...
    fwd_fixed = mne.convert_forward_solution(
        fwd_dense, surf_ori=True, force_fixed=True, use_cps=True
    )
    ...
    direction_error_curve = np.zeros(
        [len(snr), num_sim]
    )  # error in direction detection
    direction_error_sphere = np.zeros(
        [len(snr), num_sim]
    )  # error in direction detection
    ...
    for snr_level in snr:
        ...
        for sim_n in range(0, num_sim):
            ...
            [
                ...
                direction_on_sphere,
                projected_path,
            ] = create_waves_on_sensors_py(
                fwd_fixed, params, channel_type, src_idx_dense[sim_n], spherical=False
            )
            ...
            direction_generated_curve = direction_curved[
                direction_ind, speed_generated[k, sim_n], :
            ]  # direction for wave simulation
            direction_generated_pca = direction_pca[
                direction_ind, speed_generated[k, sim_n], :
            ]
            direction_generated_sphere = direction_on_sphere[
                direction_ind, speed_generated[k, sim_n], :
            ]
            projected_path_generate = projected_path[
                direction_ind, speed_generated[k, sim_n], :, :
            ]
            ...
            [
                ...
                best_coefs,
                best_shift,
                speed_estimated[k, sim_n],
            ] = LASSO_inverse_solve(data, sensor_waves, False)
            # error in speed predicted (m/s)
            # error in direction predicted (out of 1)
            direction_error_curve[k, sim_n] = 1 - abs(
                direction_curved[np.argmax(best_coefs), speed_estimated[k, sim_n], :]
                @ direction_generated_curve
                / np.linalg.norm(
                    direction_curved[
                        np.argmax(best_coefs), speed_estimated[k, sim_n], :
                    ]
                )
                / np.linalg.norm(direction_generated_curve)
            )

            direction_error_sphere[k, sim_n] = 1 - direction_on_sphere[
                np.argmax(best_coefs), speed_estimated[k, sim_n], :
            ] @ direction_generated_sphere / np.linalg.norm(
                direction_on_sphere[np.argmax(best_coefs), speed_estimated[k, sim_n], :]
            ) / np.linalg.norm(
                direction_generated_sphere
            )

            direction_error_pca[k, sim_n] = 1 - abs(
                direction_pca[np.argmax(best_coefs), speed_estimated[k, sim_n], :]
