...
def simulations_speed_error(data_dir, channel_type, params, snr, num_sim=100):
    ...
    for snr_level in snr:
        ...
        for sim_n in range(0, num_sim):
            src_idx_dense[sim_n] = np.random.randint(0, G_dense.shape[1])  # random starting source from dense cortical model
            ...
            # (with spatial error)
            [sensor_waves, direction, path_final] = create_waves_on_sensors(cortex_dense, params, G_dense,
                                                                               src_idx_dense[sim_n], spherical=False)
            # Solve the LASSO problem without intercept
            [score_fit[sim_n], best_intercept, best_coefs, best_shift, speed_estim[k, sim_n]] = \
