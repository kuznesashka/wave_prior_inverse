...
regression = ElasticNetCV(
    positive=True, normalize=True, cv=5, max_iter=10000
...
bestind = np.zeros(
    ...
    dtype=int,
)  # indices of best speeds
numdir = np.zeros(
    nspikes,
)  # number of nonzero directions in optimum
...
for ind_sp in range(0, nspikes):
    ...
    wavessp = waves[
        cumdir : cumdir + Ndir
    ]  # waves for this spike Ndir x 12S*102ch*20ms
    ...
    bestdir = np.zeros(
        S,
    )  # corresponding number of nonzero directions (without intercept)
    for s in range(0, S):
        bestdir[s] = nzdir[bestshifts[s], s]

    score_sort_ind = (-bestscore).argsort()  # indices of sorted scores for all speeds
    dir_sort_ind = (
        bestdir[score_sort_ind[0:3]]
    ).argsort()  # indices of sorted number of nonzero directions for top-3 scores
